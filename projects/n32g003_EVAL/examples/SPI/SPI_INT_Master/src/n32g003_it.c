...
extern uint16_t SPI_Master_Buffer_Tx[BufferSize];
...
{
	if(SPI_Interrupt_Flag_Status_Get(SPI_INT_FLAG_TE) != RESET)
	{
		/* Send SPI_Master_Buffer_Tx data */
		SPI_Data_Transmit(SPI_Master_Buffer_Tx[TxIdx++]);
		
		if(TxIdx == BufferSize)
		{
			/* Disable SPI_Master TE Interrupt */
			SPI_Interrupts_Disable(SPI_INT_TE);
		}
	}
}
