...
uint8_t SPI_Buffer_Rx1[BufferSize];									  
...
{
    log_init();    
    log_info("This is SPI_Slave FullDuplex Soft NSS Demo!\r\n");
    
	/* SPI configuration */
	SPI_Configuration();
	
	/* Send data */
    SPI_Data_Transmit(SPI_Buffer_Tx1[TxIdx]);
	...
	while(RxIdx < BufferSize)
	{
		...
		/* Receive data */
		SPI_Buffer_Rx1[RxIdx++] = SPI_Data_Get();
	...
	TransferStatus = Buffercmp(SPI_Buffer_Rx1, SPI_Buffer_Tx0, BufferSize);
