...
void EXTI4_17_IRQHandler(void)
{
    ...
    if (ControlFlag == 0)
    {
        /* Flush DAT register and clear the UARTz RXNE flag */
        UART_Data_Receive(UARTz);
        /* Enable the UARTz mute mode*/
        UART_Receiver_Wakeup_Enable(UARTz);

        ControlFlag = 1;
    }
    else if (ControlFlag == 1)
    {
        /* Send the address mark (0x102) to wakeup UARTz */
        UART_Data_Send(UARTy, 0x102);
